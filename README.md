...
哈希连接先在小表上 Build，再用大表 Probe。桶由标准库 unordered_map 支撑，键到 RID 列表保留重复；本主题实现的是连接的构建、探测、同键枚举及逻辑列方向恢复，不是自制哈希表。哈希冲突由容器的键相等判断处理，不能把同桶误作同键。

按输入总行数（包括 NULL）选择较小构建侧，平局选左。这是已知内存向量的精确基数选择，不是统计信息优化器；NULL 很多时并不保证实际桶占用最小。`build_rows` 和 `probe_rows` 只计非 NULL 行。不变量：构建结束后，每个非 NULL 构建行在对应键桶中恰好一次；每个非 NULL 探测行查桶一次；无论物理构建哪侧，结果始终是 `(左RID, 右RID)`。
...
- 左 4 行少于右 5 行，选择左侧 Build；跳过 NULL。
- 桶 `2 -> [0,3]`、`1 -> [2]`，实际插入 3 行。
- 右 RID 0 的 2 命中两个左 RID；RID 1 的 3 不命中；RID 2 再命中两个。
- RID 3 为 NULL 跳过；RID 4 又命中两个。Probe 非 NULL 共 4 行，输出 6 对。
- 实际探测输出按右侧推进；demo 排序仅为稳定展示，并不是哈希连接的顺序保证。
...
3,2
3,4
build=left build_rows=3 probe_rows=4
```

## 源码导读

- `src/join.h`：数据契约、统计指标与唯一入口 `tutorial::join`。
- `src/join.cpp`：从输入到结果的完整哈希连接算法，按上面不变量阅读循环。
...
设左右总行数为 L、R，输出对数为 K。在哈希均摊常数时间假设下，时间 O(L+R+K)，桶辅助空间 O(min(L,R))，输出另占 O(K)。恶意碰撞可能使哈希查找退化，不能保证最坏线性。全部同键时输出仍为 L×R。小表按行数选，不按字节数、NULL 比例或倾斜分布估算；没有分区哈希、Grace Hash 或溢写。
