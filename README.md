...
遇到 LIMIT 停止递归，保留整个子树：前缀截断会把顺序变化变成结果多重集变化。
例如 A=[1]，B=[(1,10),(1,20),(1,30),(1,40)]，连接后 LIMIT 2 原本取 payload 10、20；
标准不保证哈希等键遍历次序，改用 HashJoin 可能取到不同的两行，因此不能在此边界下替换。
...
LIMIT 回归同时检查子连接仍是原计划、选中 payload 和结果等价，不依赖本机哈希遍历顺序。
