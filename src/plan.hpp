...
enum class Kind {Scan,Filter,Project,Join,LeftJoin,Limit,Sort,HashJoin,Aggregate};
...
inline Columns schema(const P&p) {
 if(p->kind==Kind::Aggregate)return {p->leftKey,"sum"};
...
inline Rows execute(const P&p) {
 ...
 if(p->kind==Kind::Aggregate) {
  std::map<Cell,Cell> groups;
  for(auto&r:a){auto&sum=groups[r.at(p->leftKey)];auto v=r.at(p->rightKey);if(v)sum=sum.value_or(0)+*v;}
  for(auto&g:groups)out.push_back({{p->leftKey,g.first},{"sum",g.second}});
  return out;
 }
