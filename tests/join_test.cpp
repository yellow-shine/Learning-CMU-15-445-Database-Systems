...
void check(const Table &left, const Table &right) {
    ...
    require(result.index_entries == valid(right), "index entry count");
    require(result.probes == valid(left), "probe count");
    require(result.candidate_visits == result.matches.size(), "candidate RID access count");
}
int main() {
    try {
