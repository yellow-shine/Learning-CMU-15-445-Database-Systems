...
int main() {
 try {
  ...
  for(bool winner:{false,true}) {
    Temp dir;
    child([&] { Store db(dir.path); db.begin(1); db.update(1,0,10); db.commit(1); db.flushPages();
      db.begin(2); db.update(2,1,20); db.checkpoint();
      auto cp=db.loadCheckpoint(); require(cp.cutoff==3 && cp.start==3 && cp.transactions.at(2)==3 && cp.dirtyPages.at(1)==3,"saved TT/DPT");
      db.update(2,1,30); if(winner) db.commit(2); else db.flushPages(); });
    child([&] {Store db(dir.path); db.recover(); require(db.recoveryStart==3 && db.pages[0].value==10 && db.pages[1].value==(winner?30:0),"active checkpoint recovery");});
    child([&] {Store db(dir.path); db.recover(); require(db.pages[1].value==(winner?30:0),"checkpoint repeat");});
  }
  { Temp dir;
    child([&] {Store db(dir.path); db.begin(1); db.update(1,0,10); db.commit(1); db.flushPages(); db.checkpoint();
      db.begin(2); db.update(2,1,20); db.commit(2); db.checkpoint([]{::_exit(0);});});
    child([&] {Store db(dir.path); require(db.loadCheckpoint().cutoff==2,"old checkpoint remains"); db.recover(); require(db.pages[1].value==20,"checkpoint publication crash");});
  }
  { Temp dir;
    {Store db(dir.path); db.begin(1); db.update(1,0,5); db.flushPages(); db.checkpoint(); auto cp=db.loadCheckpoint(); require(cp.dirtyPages.empty() && cp.start==1,"active clean-page transaction pins start");}
    child([&] {Store db(dir.path);db.recover();require(db.pages[0].value==0,"clean stolen loser");});
