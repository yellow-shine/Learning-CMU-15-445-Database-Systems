...
int main() {
 try {
  ...
  for(const std::string phase:{"analysis","redo","clr","undo","end","pages"}) {
    Temp dir;
  ...
  {Temp dir;
    child([&] {Store db(dir.path);db.begin(1);db.update(1,0,10);db.commit(1);
      db.begin(2);db.update(2,1,20);db.update(2,1,30);db.flushPages();});
    child([&] {Store db(dir.path);db.recover([](const char* p){if(std::string(p)=="clr") ::_exit(0);});});
    {Store db(dir.path);auto r=db.log.records.back();require(r.kind==Clr && r.before==4 && r.next==3 && r.after==20,"durable CLR undoNext");require(db.pages[1].value==30,"CLR before page");}
    child([&] {Store db(dir.path);db.recover();require(db.undone==1 && db.redone==1,"redo CLR and skip compensated update");require(db.pages[1].value==0,"resume undo");});
    {Store db(dir.path);auto count=db.log.records.size();db.recover();require(db.log.records.size()==count && db.redone==0 && db.undone==0 && db.skipped==5,"PageLSN idempotence");}
  }
  {Temp dir;
    child([&] {Store db(dir.path);db.begin(1);db.update(1,0,1);db.begin(2);db.update(2,1,2);db.update(1,0,3);db.update(2,1,4);db.flushPages();});
    child([&] {Store db(dir.path);db.recover();require(db.undone==4 && db.pages[0].value==0 && db.pages[1].value==0,"interleaved loser chains");});
    {Store db(dir.path);std::vector<Word> targets;for(auto r:db.log.records)if(r.kind==Clr)targets.push_back(r.before);require(targets==std::vector<Word>{4,3,2,1},"global descending undo");}
  }
  ...
  {Temp dir;
    ...
    auto invalid=pack({Clr,3,1,0,2,5,2,0}); // Correct target, wrong undoNext (must be 1).
