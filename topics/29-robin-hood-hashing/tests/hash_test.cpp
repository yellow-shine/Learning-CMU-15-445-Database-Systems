...
int main() {
  bool rejected = false;
  try { RobinHood bad(0); } catch (const std::invalid_argument&) { rejected = true; }
  CHECK(rejected);
  RobinHood t(5);
  ...
  RobinHood swap(5);
  CHECK(swap.put(0, 0) && swap.put(1, 1) && swap.put(5, 5));
  CHECK(swap.slots()[1]->key == 5 && swap.slots()[1]->distance == 1);
  CHECK(swap.slots()[2]->key == 1 && swap.slots()[2]->distance == 1);
  CHECK(swap.erase(0));
  CHECK(swap.slots()[0]->key == 5 && swap.slots()[0]->distance == 0);
  CHECK(swap.slots()[1]->key == 1 && swap.slots()[1]->distance == 0);
  CHECK(!swap.slots()[2] && swap.valid());
  RobinHood one(1);
  CHECK(one.put(0, 0) && !one.put(1, 1) && one.erase(0) && one.valid());
  RobinHood h(31);
  std::map<int, int> oracle;
  std::mt19937 rng(2901);
  for (int n = 0; n < 12000; ++n) {
    ...
    CHECK(h.size() == oracle.size()); CHECK(h.valid());
