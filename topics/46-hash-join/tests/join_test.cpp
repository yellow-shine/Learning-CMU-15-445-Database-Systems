...
void check(const Table &left, const Table &right) {
    ...
    const auto valid = [](const Table &t) {
        return static_cast<std::size_t>(std::count_if(t.begin(), t.end(), [](Row r) { return r.has_value(); }));
    };
    require(result.build_left == (left.size() <= right.size()), "build smaller side / tie");
    require(result.build_rows == valid(result.build_left ? left : right), "build row count");
    require(result.probe_rows == valid(result.build_left ? right : left), "probe row count");
}
int main() {
    try {
